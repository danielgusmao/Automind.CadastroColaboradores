...
Identidade App-only criada e validada:
- App Registration: `Automind.CadColab`;
...
- chave privada nao exportavel em `LocalMachine\My`;
- `AUTOMIND\gMSA_CadColab$` possui somente leitura na chave privada CNG.
...
A permissao delegada padrao `User.Read` permanece, mas nao participa do fluxo App-only.

### Consulta real de licencas

A autenticacao `client_credentials` com certificado funcionou e `GET /v1.0/subscribedSkus` retornou 20 SKUs. Snapshot relevante:
- Microsoft 365 Business Standard (`O365_BUSINESS_PREMIUM`): 166 habilitadas / 149 consumidas / 17 disponiveis;
- Microsoft 365 Business Basic: 24 / 22 / 2;
- Office 365 E3 (`ENTERPRISEPACK`): 6 / 5 / 1;
- Power BI Pro: 4 / 1 / 3;
- Project Plan 3 (`PROJECTPROFESSIONAL`): 22 / 16 / 6.

Os numeros sao historicos; o sistema deve usar o Graph em tempo real.

### Padrao da coorte de referencia

Para `Automation Systems Analyst + ENGENHARIA`, excluindo `07.Outros`:
- 5 usuarios de referencia;
- `O365_BUSINESS_PREMIUM`: 5/5, atribuicao direta;
- `FLOW_FREE`: 4/5;
- `POWER_BI_STANDARD`: 1/5;
- os 5 possuem `UsageLocation=BR`;
- Business Standard sem `disabledPlans`.

### Teste real controlado com lucas.costa

Estado inicial:
- usuario sincronizado do AD (`onPremisesSyncEnabled=True`);
- `UsageLocation` vazio;
- 0 licencas.

Operacoes validadas:
1. `usageLocation` alterado para `BR` e confirmado por releitura;
2. Microsoft 365 Business Standard (`f245ecc8-75af-4f8e-b61f-27d8114de5f3`) atribuida diretamente;
3. readback: `state=Active`, `error=None`, `assignedByGroup` vazio;
4. capacidade caiu temporariamente de 17 para 16 disponiveis;
5. a licenca foi removida pelo mesmo endpoint `assignLicense`;
6. readback final: 0 licencas, Business Standard ausente;
7. capacidade voltou para 149 consumidas / 17 disponiveis.

Estado final do Lucas:
- `UsageLocation=BR`;
- 0 licencas.

Conclusao: atribuicao e remocao direta de licenca via App-only estao tecnicamente validadas, mas **nao foram integradas ao fluxo de criacao nesta versao**.

### Problema de PowerShell encontrado

Uma sessao do Windows PowerShell possuia `ServerCertificateValidationCallback` customizado, causando erro de runspace no `Invoke-RestMethod`. Para os testes, o callback foi limpo apenas na sessao e TLS 1.2 foi usado. A aplicacao v0.1.12 nao possui bypass TLS e usa validacao padrao do Windows/.NET.

### Implementacao v0.1.12

Pedido aprovado: exibir no sistema a lista de licencas e quantidades no mesmo padrao visual do Microsoft 365 Admin Center.

Implementado:
- novo `IMicrosoft365LicenseService` / `Microsoft365LicenseService`;
- autenticacao App-only por certificado, sem client secret;
- consulta de `subscribedSkus`;
- calculo de `disponiveis = enabled - consumed`;
- nomes amigaveis para SKUs conhecidos e fallback para o part number;
- cache local de 5 minutos;
- secao `04 - MICROSOFT 365` na tela de Novo colaborador;
- exibicao como `17 de 166 licencas disponiveis`;
- assinaturas suspensas identificadas;
- SKUs de capacidade equivalente a ilimitada exibidos como ilimitados;
- falha no Graph nao derruba o formulario;
- checkboxes de licenca aparecem desabilitados nesta etapa.

Configuracao:
- `Microsoft365.Enabled=true`;
- `LicenseInventoryEnabled=true`;
- `LicenseWritesEnabled=false`;
- TenantId/ClientId/thumbprint configurados;
- nenhum segredo armazenado.

### Limite de seguranca da v0.1.12

Apesar de o tenant ja possuir permissoes Graph de escrita validadas, a aplicacao desta versao **nao possui endpoint de atribuicao/remocao de licenca**. O inventario e somente leitura e `LicenseWritesEnabled=false`.

### Teste esperado no servidor

Depois de build/publish:
1. abrir `Novo colaborador`;
2. confirmar secao `04 - MICROSOFT 365`;
3. confirmar status `Graph conectado`;
4. confirmar Business Standard no formato `17 de 166 licencas disponiveis`, enquanto o tenant estiver no estado observado;
5. confirmar que checkboxes de licenca permanecem desabilitados;
6. confirmar que AD/TOPdesk continuam funcionando sem dependencia de escrita M365.

### Rollback da v0.1.12

Rollback funcional simples: definir `Automind:Microsoft365:Enabled=false` ou `LicenseInventoryEnabled=false` e publicar. Isso interrompe somente a consulta visual de licencas.

Rollback completo de Entra/certificado/ACE esta consolidado em `Docs/07-MICROSOFT-365.md` e deve ser executado uma alteracao por vez.

---

## 24/09/2026 - v0.1.11 - fase Active Directory encerrada; proxima fase Microsoft 365 somente leitura

### Fechamento real da v0.1.10

Chamado de teste ponta a ponta: `I2609-0305`.

Usuario criado pela aplicacao:
- nome: `Lucas Henrique Almeida Costa`;
- sAMAccountName: `lucas.costa`;
- DN: `CN=Lucas Henrique Almeida Costa,OU=07.Outros,OU=Automind,DC=automind,DC=com,DC=br`;
- `Enabled=True`;
- `PasswordLastSet=24/09/2026 14:50:29`;
- UPN/mail: `lucas.costa@automind.com.br`;
- manager: `CN=Edson Neto,OU=Engenharia,OU=03.UDN,OU=Automind,DC=automind,DC=com,DC=br`;
- grupos diretos: exatamente 1;
- membership direta: `_CriaMovePastas`.

A auditoria `C:\Automind.CadastroColaboradores\Logs\ProvisioningAudit.jsonl` confirmou a sequencia completa:
`provisioning-start -> create-disabled -> attributes -> password -> manager -> groups -> readback-disabled -> enable -> provisioning-complete`.
Todas as etapas esperadas concluíram com sucesso e `TechnicalIdentity=AUTOMIND\gMSA_CadColab$`.

### Teste negativo da allowlist de grupos

Foi selecionado `_Engenharia` junto com `_CriaMovePastas`. Resultado:
- `Grupos validos` ficou vermelho;
- pre-validacao foi bloqueada;
- nenhuma escrita foi executada.

Conclusao: `GroupWriteAllowedDns` esta efetivamente bloqueando grupos fora da allowlist.

### Fase Active Directory - CONCLUIDA

Validado ponta a ponta:
- criacao controlada em `07.Outros`;
- atributos;
- senha inicial;
- manager;
- sugestao de grupos por cargo/departamento;
- excecoes e busca manual;
- bloqueio de protegidos;
- allowlist de grupos;
- membership real;
- readback;
- enable final;
- auditoria;
- identidade tecnica gMSA;
- rollback de memberships preparado para remover somente associacoes criadas pela operacao.

Nao ampliar a allowlist para grupos de producao sem nova decisao explicita.

### Decisao sobre senha em PDF/impressao

Foi sugerido ocultar a senha em PDF/impressao; o responsavel determinou explicitamente **nao fazer essa alteracao**. Nao modificar este comportamento sem novo pedido. A senha continua proibida em banco, log e historico da aplicacao.

### Proxima fase

Retomar Microsoft 365 / Entra **somente em leitura**. Estado conhecido:
- Tenant ID: `9ab05ca8-1779-410b-ae61-82dbd20810f3`;
- conectividade TCP/443 e HTTPS para Entra/Graph ja validadas;
- `GET /v1.0/subscribedSkus` sem token retornou 401, confirmando endpoint acessivel;
- nenhum App Registration, segredo/certificado ou permissao Graph foi criado;
- `Microsoft365.Enabled=false` permanece.

Proximo gate: refazer preflight somente leitura no servidor `10.1.2.21` e, com o resultado, definir identidade App-only e permissao minima antes de qualquer alteracao no tenant.
...
- build da v0.1.7 na maquina do Visual Studio: **sucesso, 0 erros, 15 avisos CA1416**;
- os 15 avisos estao concentrados em `Services/AdConnectionFactory.cs` e existem porque o projeto estava em `net10.0` enquanto usa APIs `System.DirectoryServices` suportadas apenas em Windows;
- o sistema e intencionalmente Windows/IIS/Active Directory e ja aborta em runtime fora do Windows.

### Alteracao tecnica

- `TargetFramework` alterado de `net10.0` para `net10.0-windows`;
- nenhuma regra de negocio, AD, IIS, TOPdesk ou provisionamento foi alterada;
- objetivo: declarar no contrato de compilacao a plataforma real da aplicacao e eliminar corretamente CA1416, sem `#pragma`, `NoWarn` ou supressao artificial;
- proximo build esperado: **0 erros e 0 avisos CA1416**.

### Regra documental refinada pelo responsavel

- nao preservar dezenas de checkpoints separados apenas para manter arquivos antigos;
- **unificar e evoluir** a documentacao;
- `Docs/CHECKPOINT.md` e o unico checkpoint historico/cumulativo, dividido por versoes/interacoes e com o mais novo primeiro;
- conteudo de `Docs/CP-HIST.md`, `Docs/Checkpoints/*`, `Docs/CHECKPOINT-LOGIN-AD.md` e do contexto historico v0.1.3 foi incorporado neste arquivo antes da remocao dos duplicados;
- documentos tematicos (`ACTIVE-DIRECTORY`, `GRUPOS`, `IIS-GMSA`, `ROLLBACK`, etc.) continuam existindo e devem evoluir por tema;
- consolidar significa reduzir redundancia de arquivos **sem perder informacao**.

### Estado funcional mantido

- `Automind:Mode=PilotWrite`;
- escrita de usuario somente em `07.Outros`;
- `GroupWritesEnabled=false`;
- nenhuma membership real e gravada;
- grupos comuns/excecoes/outros grupos permanecem na fase de selecao/validacao;
- Microsoft 365, `proxyAddresses` e `pwdLastSet` continuam fora da escrita.

### Proximo teste

Na maquina do Visual Studio:

`git branch --show-current;dotnet build -c Release;git status --short`

Esperado: branch `release`, build sem erros e sem CA1416. Depois publicar e executar o teste funcional limpo de sugestao/selecionamento de grupos antes de habilitar memberships.

---

> Regra atual: este e o checkpoint principal. Novas secoes entram sempre no topo. O historico abaixo deve ser preservado integralmente. Nao criar novos arquivos individuais de checkpoint a cada versao. Os arquivos historicos ja existentes em `Docs/Checkpoints/` permanecem apenas como legado documental e nao devem ser apagados.

## 24/09/2026 - v0.1.7 - coorte de referencia blindada contra contas piloto

### Evidencia recebida

- build local da v0.1.6 executado no Visual Studio / .NET 10;
- resultado: **sucesso, 0 erros e 15 avisos CA1416**, mesmos avisos Windows-only ja conhecidos e aceitos para o destino IIS/Windows;
- branch exibida: `release`;
- usuario solicitou acelerar os testes e encerrar a fase atual de descoberta/selecao de grupos.

### Revisao dos testes pendentes

O historico confirma que a fase atual deve fechar:

1. sugestao por `Title + Department`;
2. grupos comuns 100% marcados;
3. excecoes desmarcadas;
4. busca manual em `Outros grupos`;
5. backend revalidando DNs e bloqueando grupos protegidos;
6. nenhuma escrita de memberships enquanto `GroupWritesEnabled=false`.

### Problema adicional identificado antes do teste limpo

A v0.1.6 excluia o proprio colaborador por `sAMAccountName` e por `CN + OU`, corrigindo o reprocessamento do mesmo piloto. Porem, uma conta piloto anterior em `07.Outros` com o mesmo `Title + Department` poderia continuar entrando na coorte quando um **novo** colaborador fosse avaliado. Isso voltaria a distorcer a frequencia 5/5 para 5/6.

### Correcao v0.1.7

- nova configuracao `Automind:Ad:SuggestionExcludedOuDns`;
- atualmente contem apenas `OU=07.Outros,OU=Automind,DC=automind,DC=com,DC=br`;
- usuarios localizados nessa OU (ou abaixo dela) nao participam da coorte estatistica de referencia;
- a pesquisa continua abrangendo todas as demais OUs abaixo de `PeopleSearchBase`;
- exclusao do proprio colaborador por login/CN+OU permanece como defesa adicional;
- nenhum grupo e criado ou alterado;
- `Automind:Mode=PilotWrite` permanece;
- `GroupWritesEnabled=false` permanece;
- writer continua sem gravar memberships.

### Plano para zerar esta fase

Apos build/deploy da v0.1.7, executar um unico teste funcional limpo, preferencialmente com um novo chamado ficticio de dados realistas e identidade ainda inexistente:

- selecionar `07.Outros`;
- buscar grupos;
- esperar 8 comuns 5/5 em verde e 4 excecoes;
- pesquisar `_Internet` em `Outros grupos`, adicionar manualmente e confirmar a classificacao manual;
- executar `Validar no AD` e confirmar validacao de grupos;
- **nao criar um colaborador real de producao nesta fase**, pois memberships ainda estao bloqueadas;
- se o teste fechar, encerrar a fase de descoberta/selecao e iniciar a fase separada de escrita de memberships.

### Usuario piloto existente

`teste.cadcolab` permanece no AD. Nao excluir agora. Como `07.Outros` passa a ser excluida da coorte de referencia, a conta piloto deixa de contaminar futuros testes de sugestao.

---

## 24/09/2026 - v0.1.6 - correcao da coorte e regra de checkpoint unico

### Pedido/decisao do responsavel

- checkpoint deve ser um unico arquivo cumulativo, dividido por versoes/interacoes, com as informacoes mais novas sempre primeiro;
- nao e necessario criar novos arquivos individuais por versao;
- nunca reduzir ou apagar o historico existente;
- toda interacao relevante, teste, decisao, resultado e pendencia deve continuar documentada para handoff a humanos ou LLMs.

### Evidencia visual recebida

PDF `Novo colaborador - Cadastro de Colaboradores _ Automind6.pdf` mostrou:

- `Automation Systems Analyst + ENGENHARIA`;
- OU `07.Outros`;
- 0 grupos classificados como comuns e 12 excecoes;
- os 8 grupos que deveriam ser comuns apareceram como `5/6`;
- campo `Outros grupos` ja aparece na tela;
- `GroupWritesEnabled=false`, portanto nenhuma membership e escrita;
- pre-validacao acusa login invalido/indisponivel e CN indisponivel, esperado porque o usuario piloto ja existe e o login exibido tem 21 caracteres.

### Causa confirmada no codigo

A v0.1.5 excluia o colaborador da coorte somente por `sAMAccountName`. O usuario criado anteriormente possui `sAMAccountName=teste.cadcolab`, mas o formulario atual voltou a exibir `teste.provisionamento`. Como os valores diferem, o usuario piloto entrou na propria coorte. Como ele ainda nao possui grupos de negocio, os 8 grupos historicamente 5/5 passaram a 5/6 e foram classificados como excecao.

### Correcao implementada na v0.1.6

- a exclusao continua por `sAMAccountName`;
- adicionalmente, quando `NomeCompleto` e `OU de destino` estao informados, o servico exclui o objeto cujo `CN` corresponda ao colaborador dentro exatamente daquela OU;
- `BuscarGrupos` envia `NomeCompleto` e `OuDistinguishedName` ao backend;
- a pre-validacao usa a mesma regra para recalcular a coorte;
- nenhuma permissao de escrita de grupo foi habilitada;
- `GroupWritesEnabled=false` permanece;
- writer continua com `GroupDns=[]`.

### Decisao sobre o usuario piloto existente

Nao excluir agora `teste.cadcolab`. Primeiro validar a correcao da coorte e da interface. A exclusao/recriacao do usuario so deve ser considerada mais tarde, quando a escrita real de memberships estiver implementada e houver autorizacao explicita para um novo teste ponta a ponta limpo.

### Resultado esperado apos publicar v0.1.6

Para `Automation Systems Analyst + ENGENHARIA`, com `07.Outros` selecionada:

- 5 usuarios de referencia;
- 8 grupos em `5/5`, verdes e marcados como `Comum ao cargo`;
- `_GAP7` em `4/5`;
- `Dist_sustentacao_engenharia`, `_GAP2` e `_Internet` em `1/5`;
- campo `Outros grupos` continua permitindo pesquisa manual no AD;
- nenhuma membership e gravada nesta versao.

### Proximo teste

1. build local .NET 10;
2. publicar normalmente pela branch `release`;
3. abrir o chamado ficticio;
4. manter OU `07.Outros`;
5. clicar `Buscar grupos no AD`;
6. confirmar `8 comuns / 4 excecoes` e incidencias `5/5`;
7. nao excluir o usuario piloto e nao habilitar escrita de grupos ainda.

---

---

# HISTORICO DETALHADO CONSOLIDADO
...
# ANEXO HISTORICO - CONTEXTO v0.1.3
...
## Registro historico incorporado de `21-REGISTRO-INTERACOES-24-09-2026.md`
